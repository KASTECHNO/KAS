...
<head>
    ...
    <title>{{ $seo['title'] ?? ($company->name ?? 'KAS Technology') }}</title>
    <meta name="description" content="{{ $seo['description'] ?? ($company->description ?? 'KAS Technology') }}">
    <meta name="keywords" content="{{ $seo['keywords'] ?? 'KAS Technology, CRM, developpement web, transformation digitale' }}">
    <meta name="robots" content="index, follow, max-image-preview:large">
    <meta name="author" content="{{ $company->name ?? 'KAS Technology' }}">
    <link rel="canonical" href="{{ $seo['canonical'] ?? url()->current() }}">
    <meta property="og:type" content="website">
    <meta property="og:locale" content="fr_FR">
    <meta property="og:site_name" content="{{ $company->name ?? 'KAS Technology' }}">
    <meta property="og:title" content="{{ $seo['title'] ?? ($company->name ?? 'KAS Technology') }}">
    <meta property="og:description" content="{{ $seo['description'] ?? ($company->description ?? 'KAS Technology') }}">
    <meta property="og:url" content="{{ $seo['canonical'] ?? url()->current() }}">
    <meta property="og:image" content="{{ $seo['image'] ?? asset('images/image.png') }}">
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ $seo['title'] ?? ($company->name ?? 'KAS Technology') }}">
    <meta name="twitter:description" content="{{ $seo['description'] ?? ($company->description ?? 'KAS Technology') }}">
    <meta name="twitter:image" content="{{ $seo['image'] ?? asset('images/image.png') }}">
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .contact-card p { margin: 0 0 10px; line-height: 1.6; }
        .contact-card a { color: var(--primary); text-decoration: none; }
        .map-link { display: inline-flex; align-items: center; gap: 8px; margin-top: 8px; padding: 9px 12px; border: 1px solid #bfdbfe; border-radius: 999px; background: #eef2ff; color: #1d4ed8; font-weight: 700; }
        .contact-note { color: var(--muted); font-size: .86rem; }
...
<main class="wrap">
    ...
    <section id="contact" class="contact">
        <article class="panel contact-card">
            ...
            <p class="contact-note">Informations structurées pour Google et recherche locale.</p>
            @if(!empty($seo['map_url']))
                <a class="map-link" href="{{ $seo['map_url'] }}" target="_blank" rel="noopener noreferrer">
